...
# START estimate SCG taxonomy time per site
SECONDS=0

anvi-estimate-scg-taxonomy -c $CONTIGS_DB -p $MERGED_PROFILE_DB --metagenome-mode --num-threads $threadsRun --simplify-taxonomy-information --compute-scg-coverages --update-profile-db-with-taxonomy
anvi-estimate-scg-taxonomy -c $CONTIGS_DB -p $MERGED_PROFILE_DB -C $COLLECTION --num-threads $threadsRun --simplify-taxonomy-information --compute-scg-coverages  -o $SCG_TAX

# END estimate SCG taxonomy time  per sample
ELAPSED="${site} $(($SECONDS / 3600))hrs $((($SECONDS / 60) % 60))min $(($SECONDS % 60))sec"
echo "$ELAPSED" >> ${mainDIR}/time-09-estimate_scg_taxonomy_per_site.tsv
