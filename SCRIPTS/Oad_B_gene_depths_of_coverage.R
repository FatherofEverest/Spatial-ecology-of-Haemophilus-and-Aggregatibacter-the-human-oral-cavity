## ---------------------------
##
## Script name: Oad_B_gene_depths_of_coverage.R
##
...
# grep_result=$(grep 'Na+-transporting oxaloacetate/methylmalonyl-CoA decarboxylase, beta subunit (OadB)' "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-gene_calls.txt")
# if [ -z "$grep_result" ]; then
# echo "NA" > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-OadB-gene_caller_ID.txt"
# else
#   echo "$grep_result" | awk -F"\t" '{print $1}' > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-OadB-gene_caller_ID.txt"
# fi
# done
# done
...
for (oral_site in site_list) {
  ...
  gene_coverage_df_filtered_1 <- gene_coverage_df %>% 
    dplyr::filter(gene_callers_id == gene_caller_ID_OadB) %>% # select focal gene of interest
...
for (oral_site in site_list) {
  ...
  for (genome in genome_list) {
    ...
    gene_caller_ID_df <- read.table(paste0("/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_",oral_site,"-profile/bin_by_bin/",genome,"/",genome,"-OadB-gene_caller_ID.txt"), header = FALSE)       
...
OadB_average_cov <- genomes_gene_coverages_combo_average %>% 
  dplyr::select(mean_gene_coverage) %>% 
  dplyr::rename(OadB_average_cov = mean_gene_coverage)

final_df <- cbind(final_df, OadB_average_cov)
...
df_tidy <- final_df %>%
  select(sample, site, species_coverage, OadB_average_cov) %>% 
  gather(key = "Category", value = "Value", species_coverage, OadB_average_cov)
...
Gene_and_focal_genome_cov_plot <- ggplot(df_tidy_focal, aes(x = sample, y = Value, fill = Category)) +
  ...
  scale_fill_manual(values = c("blue", "red"))+
  xlab(NULL)+
  ylab("Mean coverage of H. parainfluenzae TD specialist genome & OadB gene") +
...
# save final plot
ggsave("/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_B_gene_genome_species_coverage_plot.pdf",final_plot_version_2, width = 10, height = 6)

# Save data
write.csv(df_tidy_focal, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_B_focal_genome_coverage.csv", quote = FALSE, row.names = FALSE)
write.csv(df_tidy, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_B_species_coverage.csv", quote = FALSE, row.names = FALSE)
