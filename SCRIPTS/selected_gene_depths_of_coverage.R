## ---------------------------
##
## Script name: selected_gene_depths_of_coverage.R
##
...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### 
#### The followiong was run in bash environment to create the gene caller ID files for the OadG gene 
...
# grep_result=$(grep 'Na+-transporting oxaloacetate/methylmalonyl-CoA decarboxylase, gamma subunit (OadG)' "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-gene_calls.txt")
# if [ -z "$grep_result" ]; then
# echo "NA" > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-OadG-gene_caller_ID.txt"
# else
#   echo "$grep_result" | awk -F"\t" '{print $1}' > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-OadG-gene_caller_ID.txt"
# fi
# done
# done
...
# for site in PP BM TD; do 
# for genome in $(cat /Volumes/JJG_Ext_SSD/metapangenomics/genome_list.txt); do
# grep_result=$(grep 'Biotin carboxyl carrier protein (AccB) (PDB:1A6X)!!!Pyruvate/oxaloacetate carboxyltransferase (OadA1) (PDB:2NX9) (PUBMED:18681940)' "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-gene_calls.txt")
# if [ -z "$grep_result" ]; then
# echo "NA" > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-OadA1-gene_caller_ID.txt"
# else
#   echo "$grep_result" | awk -F"\t" '{print $1}' > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-OadA1-gene_caller_ID.txt"
# fi
# done
# done
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####  
...
#gene_caller_ID_OadA1 <- "20021"
#gene_caller_ID_OadG <- "20020"
#gene_caller_ID_OadB <- "20022"

# focal genome ID
focal_genome_ID <- "H_parainfluenzae_str_M1C142_1_id_GCA_014931375_1"
...
for (oral_site in site_list) {
  ...
  gene_coverage_df_filtered_1 <- gene_coverage_df %>% 
    dplyr::filter(gene_callers_id == gene_caller_ID_OadG) %>% # select focal gene of interest
...
# Now to build the plot
  species_coverage_plot <- ggplot(final_df, aes(x = reorder(sample, -gene_coverage), y = species_coverage)) +
    facet_grid(. ~ site, scales = "free", space = "free", switch = "x") + 
    geom_bar(stat = 'identity',position = dodge, fill = "navyblue") +
    scale_y_continuous(expand = c(0, 0), limits=c(0, 10), breaks=seq(0,10,by=2), oob=squish, labels = scales::comma) +
    xlab(NULL)+
    ylab("Species mean coverage") +
    theme_classic()+
  ...
  genome_coverage_plot <- ggplot(final_df, aes(x = reorder(sample, -gene_coverage), y = genome_coverage)) +
    facet_grid(. ~ site, scales = "free", space = "free", switch = "x") + 
    geom_bar(stat = 'identity',position = dodge, fill = "navyblue") +
    scale_y_continuous(expand = c(0, 0), limits=c(0, 20), breaks=seq(0,20,by=5), oob=squish, labels = scales::comma) +
    xlab(NULL)+
    ylab("Genome mean coverage") +
    theme_classic()+
    theme(text = element_text(size = 12),
          axis.ticks.y = element_line(linewidth = 1),
          axis.text.x = element_blank(),
  ...
  gene_coverage_plot <- ggplot(final_df, aes(x = reorder(sample, -gene_coverage), y = gene_coverage)) +
    facet_grid(. ~ site, scales = "free", space = "free", switch = "x") + 
    geom_bar(stat = 'identity',position = dodge, fill = "navyblue") +
    scale_y_continuous(expand = c(0, 0), limits=c(0, 100), breaks=seq(0,100,by=20), oob=squish, labels = scales::comma) +
    xlab(NULL)+
    ylab("Gene mean coverage") +
    theme_classic()+
    theme(text = element_text(size = 12),
          axis.ticks.y = element_line(linewidth = 1),
          axis.text.x = element_blank(),
  ...
  final_plot <- egg::ggarrange(gene_coverage_plot,
                               genome_coverage_plot,
                               species_coverage_plot,
                               ncol = 1,
                               heights = c(0.25,0.25,0.25))
  
  # save final plot
  ggsave("/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_A1_gene_genome_species_coverage_plot.pdf",final_plot, width = 8, height = 4)
 ...
 #####  Gene coverage averaged across all H. para genomes  #####  
  ...
  for (oral_site in site_list) {
    ...
    for (genome in genome_list) {
      ...
      # load gene caller ID for OadG gene for each genome
      gene_caller_ID_df <- read.table(paste0("/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_",oral_site,"-profile/bin_by_bin/",genome,"/",genome,"-OadG-gene_caller_ID.txt"), header = FALSE)       
      
      gene_caller_ID <- gene_caller_ID_df[,1]
      
      # get gene coverage for focal gene (e.g. OadG)
      gene_coverage_df_filtered_1 <- gene_coverage_df %>% 
  ...
  OadG_average_cov <- genomes_gene_coverages_combo_average %>% 
    dplyr::select(mean_gene_coverage) %>% 
    dplyr::rename(OadG_average_cov = mean_gene_coverage)
  
  final_df <- cbind(final_df, OadG_average_cov)
  ...
  df_tidy <- final_df %>%
    select(sample, site, species_coverage, OadG_average_cov) %>% 
    gather(key = "Category", value = "Value", species_coverage, OadG_average_cov)
  ...
  Gene_and_focal_genome_cov_plot <- ggplot(df_tidy_focal, aes(x = sample, y = Value, fill = Category)) +
    ...
    scale_fill_manual(values = c("blue", "red"))+
    xlab(NULL)+
    ylab("Mean coverage of H. parainfluenzae TD specialist genome & OadG gene") +
    labs(y = paste0("Mean coverage of", "\n", "H. parainfluenzae TD specialist genome", "\n", 
                    "&", "\n", "OadG gene"),
         x = NULL) +
    theme_classic() +
    theme(text = element_text(size = 12),
  ...
  # save final plot
  ggsave("/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_G_gene_genome_species_coverage_plot.pdf",final_plot_version_2, width = 10, height = 6)
  
  # Save data
  write.csv(df_tidy_focal, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_G_focal_genome_coverage.csv", quote = FALSE, row.names = FALSE)
  write.csv(df_tidy, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_G_species_coverage.csv", quote = FALSE, row.names = FALSE)
