## ---------------------------
##
## Script name: Oad_A1_gene_depths_of_coverage.R
##
...
for (oral_site in site_list) {
  ...
  gene_coverage_df_filtered_1 <- gene_coverage_df %>% 
    dplyr::filter(gene_callers_id == gene_caller_ID_OadA1) %>% # select focal gene of interest
...
for (oral_site in site_list) {
  ...
  for (genome in genome_list) {
    ...
    gene_caller_ID_df <- read.table(paste0("/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_",oral_site,"-profile/bin_by_bin/",genome,"/",genome,"-OadA1-gene_caller_ID.txt"), header = FALSE)       
...
OadA1_average_cov <- genomes_gene_coverages_combo_average %>% 
  dplyr::select(mean_gene_coverage) %>% 
  dplyr::rename(OadA1_average_cov = mean_gene_coverage)

final_df <- cbind(final_df, OadA1_average_cov)
...
df_tidy <- final_df %>%
  select(sample, site, species_coverage, OadA1_average_cov) %>% 
  gather(key = "Category", value = "Value", species_coverage, OadA1_average_cov)

# Calculate the order based on gene_coverage values
sample_order <- df_tidy %>%
  dplyr::filter(Category == "species_coverage") %>%
  arrange(desc(Value)) %>%
  pull(sample)
...
Gene_and_focal_genome_cov_plot <- ggplot(df_tidy_focal, aes(x = sample, y = Value, fill = Category)) +
  ...
  scale_fill_manual(values = c("blue", "red"))+
  xlab(NULL)+
  ylab("Mean coverage of H. parainfluenzae TD specialist genome & OadA1 gene") +
...
# save final plot
ggsave("/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_A1_gene_genome_species_coverage_plot.pdf",final_plot_version_2, width = 10, height = 6)

# Save data
write.csv(df_tidy_focal, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_A1_focal_genome_coverage.csv", quote = FALSE, row.names = FALSE)
write.csv(df_tidy, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/OAD_A1_species_coverage.csv", quote = FALSE, row.names = FALSE)
