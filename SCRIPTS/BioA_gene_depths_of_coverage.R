## ---------------------------
##
## Script name: BioA_gene_depths_of_coverage.R
##
...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### 
#### The followiong was run in bash environment to create the gene caller ID files for the BioA gene 
...
grep_result=$(grep 'Adenosylmethionine-8-amino-7-oxononanoate aminotransferase (BioA) (PDB:1MLY)' "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-gene_calls.txt")
if [ -z "$grep_result" ]; then
echo "NA" > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-BioA-gene_caller_ID.txt"
else
  echo "$grep_result" | awk -F"\t" '{print $1}' > "/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_$site-profile/bin_by_bin/$genome/$genome-BioA-gene_caller_ID.txt"
fi
done
done
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####  
...
for (oral_site in site_list) {
  ...
  gene_coverage_df_filtered_1 <- gene_coverage_df %>% 
    dplyr::filter(gene_callers_id == gene_caller_ID_BioA) %>% # select focal gene of interest
...
for (oral_site in site_list) {
  ...
  for (genome in genome_list) {
    ...
    # load gene caller ID for BioA gene for each genome
    gene_caller_ID_df <- read.table(paste0("/Volumes/JJG_Ext_SSD/metapangenomics/P_0622_Haemophilus_Aggregatibacter_",oral_site,"-profile/bin_by_bin/",genome,"/",genome,"-BioA-gene_caller_ID.txt"), header = FALSE)       
    
    gene_caller_ID <- gene_caller_ID_df[1,1]
    
    # get gene coverage for focal gene (e.g. BioA)
    gene_coverage_df_filtered_1 <- gene_coverage_df %>% 
...
BioA_average_cov <- genomes_gene_coverages_combo_average %>% 
  dplyr::select(mean_gene_coverage) %>% 
  dplyr::rename(BioA_average_cov = mean_gene_coverage)

final_df <- cbind(final_df, BioA_average_cov)
...
df_tidy <- final_df %>%
  select(sample, site, species_coverage, BioA_average_cov) %>% 
  gather(key = "Category", value = "Value", species_coverage, BioA_average_cov)
...
# save final plot
ggsave("/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/BioA_gene_genome_species_coverage_plot.pdf",final_plot_version_2, width = 10, height = 6)

# Save data
write.csv(df_tidy_focal, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/BioA_focal_genome_coverage.csv", quote = FALSE, row.names = FALSE)
write.csv(df_tidy, file = "/Users/home/SPECIES_LEVEL_PANGENOMES/Haemophilus_and_Aggregatibacter/FIGURES/Gene_level/BioA_species_coverage.csv", quote = FALSE, row.names = FALSE)
